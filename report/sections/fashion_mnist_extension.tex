...
\input{tables/fashion_mnist_problem_setup}
...
\input{tables/fashion_mnist_naive_results}
...
\input{tables/fashion_mnist_naive_top_errors}
...
\input{tables/fashion_mnist_single_branch_results}
...
\input{tables/fashion_mnist_top_errors}
...
\input{tables/fashion_mnist_two_branch_results}
...
\input{tables/fashion_mnist_two_branch_top_errors}
...
\input{tables/fashion_mnist_ensemble_member_details}
\input{tables/fashion_mnist_seed_comparison}
\input{tables/fashion_mnist_ensemble_results}
...
\input{tables/fashion_mnist_ensemble_top_errors}
...
\input{tables/fashion_mnist_region_fusion_seed_comparison}
\input{tables/fashion_mnist_region_fusion_results}
...
\input{tables/fashion_mnist_region_fusion_top_errors}
...
\input{tables/fashion_ann_architecture_details}
\input{tables/fashion_ann_hyperparameters}
\input{tables/fashion_ann_iteration_logic}
\input{tables/fashion_mnist_model_comparison}
...
Kết luận cho Chương 2 cũng có thể rút lại thành ba ý rõ ràng. Thứ nhất, raw pixel thuần là chưa đủ cho FashionMNIST; baseline naive dừng ở $89.59\%$, trong khi chỉ riêng bước thêm \textit{deslant + recenter + HOG} đã nâng kết quả lên $90.60\%$. Thứ hai, cấu hình tốt nhất hiện tại không phải two-branch hay region-focused feature fusion, mà là \textit{ensemble của ba mô hình single-branch ANN} với $91.68\%$, cho thấy trên bộ dữ liệu này việc giảm phương sai dự đoán hiệu quả hơn việc tiếp tục làm kiến trúc phức tạp hơn. Thứ ba, các đặc trưng vùng thân trên có cơ sở trực giác tốt và giúp phân tích cặp lỗi \textit{T-shirt/top} $\leftrightarrow$ \textit{Shirt} chặt chẽ hơn, nhưng trong giới hạn độ phân giải $28 \times 28$ chúng chưa tạo ra cải thiện đủ mạnh để vượt ensemble gốc. Vì vậy, insight quan trọng nhất của chương này là: với FashionMNIST, nút thắt chính không còn là thiếu đặc trưng toàn cục, mà là độ chồng lấn hình dáng giữa các lớp; trong bối cảnh đó, ensemble là bước cải tiến đáng giữ, còn các hướng feature engineering cục bộ mạnh hơn vẫn cần được kiểm chứng thêm.
