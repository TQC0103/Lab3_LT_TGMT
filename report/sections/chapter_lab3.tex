...
\input{tables/mnist_problem_setup}
...
\input{tables/mnist_naive_error_by_class}
...
\input{tables/mnist_ann_architecture_details}
\input{tables/mnist_ann_hyperparameters}
\input{tables/mnist_ann_iteration_logic}
\input{tables/mnist_ann_model_comparison}
...
Kết luận cho Chương 1 là: mô hình tốt nhất theo test accuracy trong loạt thử nghiệm hiện tại là \textit{Single-branch ANN} với $99.14\%$. Hai nhận xét thực nghiệm quan trọng cần được rút ra rõ ràng. Thứ nhất, augmentation cục bộ không còn cần thiết trên MNIST sau khi dữ liệu đã được chuẩn hóa bằng \textit{deslant + recenter}, vì bộ dữ liệu này vốn khá sạch và ít biến thiên hình học phức tạp; kết quả $99.11\%$ của biến thể có augmentation gần như không cải thiện gì so với single-branch gốc. Thứ hai, việc cố tách raw pixel và HOG thành hai nhánh riêng cũng không mang lại lợi ích thực nghiệm rõ rệt; two-branch ANN chỉ đạt $99.10\%$, thấp hơn nhẹ so với single-branch, cho thấy trong bối cảnh MNIST thì việc hợp nhất sớm raw pixel và HOG đã đủ tốt. Nói cách khác, giá trị lớn nhất của chuỗi thử nghiệm không chỉ nằm ở con số cuối, mà nằm ở việc nó chỉ ra khá rõ đâu là thành phần thật sự cần thiết và đâu là thành phần không còn tạo thêm lợi ích đáng kể trên MNIST.
